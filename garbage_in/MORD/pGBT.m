function [pbeta,F,pbetacrit] = pGBT(y,x,tj,fs,alpha)
%
% Productory local Beta test.
%
% Extension of the Beta test to multi channel analysis as the productory of
...
%Sintaxes:
%
%pbeta = pGBT(y,x,tj) => pGBT spectrum.
%[pbeta,F] = pGBT(y,x,tj,fs) => pGBT spectrum plus frequency vector.
%[pbeta,F,pbetacrit] = pGBT(y,tj,x,fs,alpha) =>  returns also the simulated
%critical value.
%
%Input Parameters:
%
...
%
% Example: pGBT using two signals
%
% y1 = awgn(sin(2*pi*21*(linspace(0,10,1000))),-15,'measured','db')';
% y2 = awgn(sin(2*pi*21*(linspace(0,10,1000))),-16,'measured','db')';
% x = randn(1000,2);
% [pbeta,F,pbetacrit] = pGBT([y1 y2],x,100,100,0.05);
% figure;plot(F,pbeta,'b',21,1.01*pbeta(22),'-rv', [0 50],[pbetacrit pbetacrit],'k--')
% axis([0 F(end) 0 1]);xlabel('Frequency (Hz)');ylabel('pGBT')
...
% y = randn(nfft*M,2);
% x = randn(nfft*Mx,2);
% [pbetaN,F,pGFTcrit] = pGFT(y,x,nfft,nfft,0.05);
% figure;plot(F,pbetaN,'b',[0 F(end)],[pGFTcrit pGFTcrit],'k--')
% axis([0 F(end) 0 1]);xlabel('Frequency(Hz)');ylabel('MGFT')
% text(10,min(pGFTcrit*1.4,.98), ['FA = ' num2str(mean(double(pbetaN>pGFTcrit))*100) '%'],'fontsize',12)
% fprintf('\nFA = %f\n', mean(double(pbetaN>pGFTcrit)))
% ------------------------------------------------------------
...
if nargin > 3
    ...
    if nargin == 5
        ...
        aux = (prod(y,2)).^(1/N);
        pbetacrit= quantile(aux,1-alpha);
