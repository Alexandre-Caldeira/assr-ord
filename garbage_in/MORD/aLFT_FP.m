function [FyN, FyNcrit] = aLFT_FP(y,L,fs,alpha,fo)
%
...
% Compute F value
FyN = mean((Yfo.^2)./(1/L*sum(Yfn.^2,1)));
...
if nargout>1
    [FyNcrit]=aLFTcrit(N,L,alpha);
end

end

function [aLFTcrit]=aLFTcrit(N,L,alpha)
...
aLFT = sum(y,2); 
aLFTcrit = quantile(aLFT,1-alpha);
end
