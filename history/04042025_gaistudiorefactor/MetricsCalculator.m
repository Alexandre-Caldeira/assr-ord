classdef MetricsCalculator < handle
    ...
    methods
        ...
        function displaySummary(obj, desiredAlpha)
            % Displays summary and validates FP rate.
             ...
             if ~isempty(desiredAlpha) && ismember('FP_Rate', obj.resultsTable.Properties.VariableNames) && ismember('FP_Rate', properties(obj.resultsTable)) && ~isnan(obj.avg_fp_rate)
                 fprintf('Validation: Desired Alpha = %.4f, Calculated Avg FP Rate = %.4f (%.2f%%)\n', desiredAlpha, obj.avg_fp_rate / 100, obj.avg_fp_rate);
                 groupingVars = {'noiseMean', 'subject', 'stimulus'}; groupVarFound = '';
                 for gv = groupingVars, if ismember(gv{1}, obj.resultsTable.Properties.VariableNames), groupVarFound = gv{1}; break; end, end
                 if ~isempty(groupVarFound) % Check FP rate per group
                     try
                        groupStats = groupsummary(obj.resultsTable, groupVarFound, 'mean', 'FP_Rate', 'IncludeEmptyGroups', false);
                        ...
                        if any(fp_dev > 0.015), warning('FP rate in one or more groups deviates significantly from desired alpha!'); end % Increased tolerance
                     catch ME_group, warning('Could not calculate grouped FP rate summary: %s', ME_group.message); end
                 elseif abs(obj.avg_fp_rate / 100 - desiredAlpha) > 0.015 % Check overall if no grouping
                     warning('Calculated overall average FP rate deviates significantly from desired alpha!');
                 elseif ~isnan(obj.avg_fp_rate), fprintf(' -> Overall FP rate matches desired alpha within tolerance.\n'); end
             elseif ~isempty(desiredAlpha) && ismember('FP_Rate', properties(obj.resultsTable)) && isnan(obj.avg_fp_rate) && obj.nNoiseFreqs > 0, warning('FP Rate is NaN, cannot validate against desired alpha (%.4f).', desiredAlpha);
             elseif ~isempty(desiredAlpha) && ~ismember('FP_Rate', properties(obj.resultsTable)), warning('FP_Rate column not found in results table for validation.');
             end
             fprintf('-------------------------------------------\n');
        end
        ...
        function plotParetoFront(obj, xMetric, yMetric, figHandle, options)
            % Plots Pareto front with optional labels and M-coloring.
             arguments
                 ...
                 options.markerSizeData (1,1) {mustBePositive} = 25
                 options.markerSizeFront (1,1) {mustBePositive} = 50
                 options.markerAlphaData (1,1) {mustBeInRange(options.markerAlphaData, 0, 1)} = 0.8
             end
             ...
             if useColor && ~isempty(colorDataValid)
                 ...
                 plot(ax, xDataValid, yDataValid, 'k.', 'MarkerSize', options.markerSizeData * 0.5, 'DisplayName', 'All Valid Parameter Sets');
             end
             ...
             if useColor && ~isempty(sortedColorDataPareto)
                 ...
                 plot(ax, sortedX, sortedY, 'ob', 'MarkerFaceColor', 'b', 'MarkerEdgeColor', 'k', 'LineWidth', 1.0, 'MarkerSize', sqrt(options.markerSizeFront), 'DisplayName', 'Pareto Front');
             end
             ...
             if options.addLabels
                 ...
                 for i = 1:numel(sortedX)
                     ...
                     if ismember('stepSize', row.Properties.VariableNames) && ~isnan(row.stepSize) , labelParts{end+1} = sprintf('SS=%d', row.stepSize); end % Show stepSize if relevant
        ...
        function plotGroupedParetoFront(obj, groupingVar, xMetric, yMetric, figHandle, options)
            % Plots multiple Pareto fronts, colored by M, grouped by groupingVar.
             arguments
                 ...
                 options.colormapName char = 'jet'
                 options.markerSizeFront (1,1) {mustBePositive} = 36
             end
             ...
             groupLineColors = parula(numGroups); % Use perceptually uniform colormap
             ...
             for i = 1:numGroups
                 ...
                 if ~isempty(paretoX)
                     [sortedX, sort_idx] = sort(paretoX); sortedY = paretoY(sort_idx);
                     sortedOriginalIndicesGroup = originalFrontIndices(sort_idx); % Indices within groupTable
                     ...
                     if useColor
                         ...
                         allColorData = [allColorData; colorDataParetoGroup]; % Collect for common color axis
                         scatter(ax, sortedX, sortedY, options.markerSizeFront, colorDataParetoGroup, 'filled', ...
                                 'MarkerEdgeColor', groupLineColors(i,:)*0.7, 'LineWidth', 0.7, ...
                     ...
                     legendEntries{i} = sprintf('%s = %g (No Front)', groupingVar, groupVal); % Indicate missing front in legend
                 end
             end
        ...
        function plotAllMetrics(obj, options)
            ...
            arguments
                ...
                options.includeParetoTimeTP logical = true % Plot TP(X) vs AvgTime(Y)
            ...
            if options.includeBoxcharts
             ...
             if options.includeParetoTimeTP && ismember('AvgDetectTime_s', obj.resultsTable.Properties.VariableNames)
                 % Plot TP Rate (X) vs AvgDetectTime (Y)
                 obj.plotParetoFront('TP_Rate', 'AvgDetectTime_s', figure(baseFigNum + figCount), ...
                      'addLabels', options.addParetoLabels, 'colormapName', options.paretoColormap);
                 figCount = figCount + 1;
             end
             if options.includeGroupedPareto
                 if ismember(options.groupingVar, obj.resultsTable.Properties.VariableNames) && ismember('AvgDetectTime_s', obj.resultsTable.Properties.VariableNames)
                     % Plot grouped TP Rate (X) vs AvgDetectTime (Y)
                     obj.plotGroupedParetoFront(options.groupingVar, 'TP_Rate', 'AvgDetectTime_s', figure(baseFigNum + figCount), ...
    ...
    methods (Access = private)
        function [paretoX, paretoY, xDataValid, yDataValid, originalFrontIndices] = calculateParetoData(obj, xMetric, yMetric, dataTable)
            ...
            valid_idx_numeric = find(valid_idx_logical); % Indices relative to input dataTable
            ...
            try
                [frontIndicesRel, ~] = paretoFront([obj1, obj2]); % Indices relative to xDataValid/yDataValid
