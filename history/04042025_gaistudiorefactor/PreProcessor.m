    ...
    methods
        ...
        function filteredSignals = applyAntunesFiltering(obj, processedSignals)
            ...
            for ch = 1:nChannels
                ...
                %    catch ME_filt, warning('Filtfilt error on chan %d, win %d: %s. Skipping window.', ch, win, ME_filt.message); filtered_x_ch(:, win) = x_ch(:, win); end
                % end
                % filteredSignals(:, :, ch) = filtered_x_ch;

                filtered_x_ch = filtfilt(b, a, x_ch')'; %filtfilt(b, a, x_ch(:, win));
                filteredSignals(:, :, ch) = filtered_x_ch;
            end
        end
