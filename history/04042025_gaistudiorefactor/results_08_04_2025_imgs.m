...
config.sim_durations = [150];        % Simulation duration in seconds
config.sim_noiseMeans = round(linspace(-40,-30,20),0);  % Mean SNR dB values
config.sim_noiseStds = [.1 1 5] ;%[.1 .5 1 1.5 2 2.5 5].^2;         % Std Dev SNR dB values
...
config.ord_startWindows =  fix(linspace(1,150,50)); % Starting window indices
% config.ord_K_stages = 12; %fix(linspace(2,10,8));      % Number of stages (K) values
config.ord_method = 'fixedStep';   % 'fixedK' or 'fixedStep'
config.ord_stepSize = fix(linspace(24, 120,10));
% [24 40 60 100];     % Only used if method = 'fixedStep'
