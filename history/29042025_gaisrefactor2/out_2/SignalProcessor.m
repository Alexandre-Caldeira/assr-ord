    ...
    methods
        ...
        function processedSignal = applyPreprocessing(obj, rawSignal, Mmax_limit)
             ...
             if obj.ZanoteliEnable
                 ...
                 for chan = 1:numChanToProcess
                     ...
                     if windowsToKeep > 0
                         ...
                         % warning('  Zanoteli: No windows below threshold for channel %d after removing first 2.', chan); % Less verbose
             ...
             if obj.FilterEnable
                 ...
                 % fprintf('  Applying Butterworth filter (Order=%d, Fc=[%.1f, %.1f] Hz)...\n', obj.FilterOrder, obj.FilterFcLower, obj.FilterFcUpper); % Less verbose
         ...
         function fftSignal = applyFFT(obj, processedSignal)
            ...
            if nChan == 0 || nWindows == 0 || nSamples == 0
                % fprintf('applyFFT: Input processedSignal has zero dimension (%d x %d x %d).\n', nSamples, nWindows, nChan); % Less verbose
                return;
            end

             numChanToProcess = nChan;
             if nChan ~= obj.NumChannels
                % warning('applyFFT: Channel count in data (%d) differs from processor config (%d). Processing %d channels.', nChan, obj.NumChannels, nChan); % Less verbose
             end

            fftSignal = zeros(obj.NumBins, nWindows, numChanToProcess); % Preallocate

            % fprintf('  Applying FFT (NFFT=%d, NumBins=%d)...\n', obj.NFFT, obj.NumBins); % Less verbose
            for chan = 1:numChanToProcess
                for epoch = 1:nWindows
                    epochData = squeeze(processedSignal(:, epoch, chan));
                    if any(isnan(epochData))
                        % warning('  FFT: NaNs found in data for epoch %d, channel %d. FFT result will be NaN.', epoch, chan); % Less verbose
                        temp = nan(obj.NFFT, 1);
                    else
                         try
         ...
         function obj = processBulkData(obj, dataManager)
            ...
            for idx1 = 1:nDim1 % Iterate stimulus/mean index
                 ...
                 if strcmp(dataManager.Mode,'exp')
                    ...
                    if ~isempty(mmax_val); suggestedMmax = mmax_val; else; needs_mmax_calc = true; end
                 else; needs_mmax_calc = true; end
                 ...
                 if needs_mmax_calc
                      ...
                      if max_windows_group > 0; suggestedMmax = max_windows_group; else; suggestedMmax = Inf; end
                 end

                for idx2 = 1:nDim2 % Iterate subject/std index
                    processedCount = processedCount + 1;
                    % fprintf('Processing Exam %d/%d (Index1: %d, Index2: %d)...\n', processedCount, totalExams, idx1, idx2); % Very verbose
                    ...
                    fftData = obj.applyFFT(processedData);
                    obj.frequencyDomainData{idx1, idx2} = fftData;
                    % fprintf('  Finished Exam %d/%d.\n', processedCount, totalExams); % Less verbose
                    if mod(processedCount, 50) == 0 % Progress indicator every 50 exams
                         fprintf('  Processed %d / %d exams...\n', processedCount, totalExams);
                    end
