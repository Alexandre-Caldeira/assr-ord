...
classdef DataManager
    ...
    methods
        ...
        function obj = loadBulkExperiments(obj, subjectIndices, stimulusIndices)
            ...
            for stimIdx = stimulusIndices(:)' % Ensure row vector
                for subjIdx = subjectIndices(:)' % Ensure row vector
                    ...
                    try
                        ...
                        % fprintf('  Loaded: Subject %d (%s), Stimulus %d (%s)\n', subjIdx, subjectID, stimIdx, stimulusName); % Verbose
                    catch ME
            ...
            fprintf('DataManager: Bulk loading complete. %d Subjects, %d Stimuli. Fs = %d Hz.\n', numel(subjectIndices), numel(stimulusIndices), obj.Fs);
        end
        ...
        function obj = generateSingleSimulation(obj, p)
            ...
            %         numEpochs, p.numChannels, p.noiseMean); % Less verbose
         end
         ...
         function obj = generateBulkSimulations(obj, groupNoiseMeans, groupNoiseStds, p)
            ...
            for i = 1:numMeans
                ...
                for j = 1:numStds
                    ...
                    % fprintf('  Generated Sim %d/%d: Mean=%.1f, Std=%.1f\n', simCount, totalSims, meanVal, stdVal); % Verbose
                end
                 if mod(i, round(numMeans/10))==0 || i==numMeans % Progress update
                      fprintf('  Finished generating for Mean Idx %d/%d (%d simulations total).\n', i, numMeans, simCount);
                 end
            end
            obj.isBulkLoaded = true;
         ...
         function data = getTimeSeries(obj, idx1, idx2)
             ...
             if ~obj.isBulkLoaded
                 warning('DataManager:getTimeSeries','Bulk data not loaded. Returning currentExamTimeSeries if available.');
             ...
             if idx1 >= 1 && idx1 <= size(obj.rawTimeSeries, 1) && idx2 >= 1 && idx2 <= size(obj.rawTimeSeries, 2)
                 ...
                 warning('DataManager:getTimeSeries','Indices (%d, %d) out of bounds for loaded bulk data [%d x %d].', ...
         ...
         function bulkData = getBulkTimeSeries(obj)
             ...
             if ~obj.isBulkLoaded
                 warning('DataManager:getBulkTimeSeries','Bulk data not loaded. Returning empty cell.');
                 bulkData = {};
             else
         ...
         function [dim1Indices, dim2Indices] = getLoadedIndices(obj)
             % Returns the indices corresponding to the loaded/generated dimensions.
             % Dim1: Stimulus index (exp) or Noise Mean index (sim)
             % Dim2: Subject index (exp) or Noise Std index (sim)
             if ~obj.isBulkLoaded
                  dim1Indices = []; dim2Indices = [];
                  warning('DataManager:getLoadedIndices','No bulk data loaded, returning empty indices.');
             elseif strcmp(obj.Mode,'exp')
                 dim1Indices = obj.loadedStimulusIndices;
                 dim2Indices = obj.loadedSubjectIndices;
             else % sim
                 dim1Indices = 1:numel(obj.simNoiseMeans); % Represent mean index
                 dim2Indices = 1:numel(obj.simNoiseStds); % Represent std index
             end
         end
