classdef ORDCalculator < handle
    ...
    methods
        ...
        function [msc, epochs, M, K] = calculateMSC(obj, processedSignals, options)
            ...
            % Reduced verbosity
            % fprintf('[%s] Calculating MSC: Method=%s, K=%d stages, M=%d windows/stage.\n', datetime, options.method, K, M);
            ...
            for ch = 1:nChannels
                ...
                for k_stage = 1:K
                    ...
                    valid_den = msc_den > eps; % Use machine epsilon
                    msc(valid_den, k_stage, ch) = msc_num(valid_den) ./ msc_den(valid_den);
                    % Leave as 0 where denominator is too small
                end
            end

            msc(isnan(msc)) = 0; % Should be handled by eps check, but just in case
            msc(isinf(msc)) = 0;
        ...
        function [epochs, M, K] = defineEpochs(obj, method, startWindow, maxWindow, stepSize, K_target)
            ...
            switch method
                ...
                case 'fixedK'
                    ...
                    if any(actualM < 2), warning('Epoch definition for fixedK=%d resulted in stages with M < 2.', K); end % Let calculateMSC handle return
                    M = floor(mean(actualM));
                    % Reduced verbosity: if std(actualM) > 1e-6, fprintf('  Note: Window distribution for fixedK resulted in varying M per stage (approx M=%d).\n', M); end
