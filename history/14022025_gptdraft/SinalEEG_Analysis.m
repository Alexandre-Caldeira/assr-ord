...
% % Author: [GPT] % Date: [14 02 2025 10:40]
...
params.alpha = 0.1;           % Overall false-positive rate
% params.Mmin = []; params.Mmax = [];
params.K     = 5;              % Number of sequential stages
params.M     = 90 / params.K;  % Window length (seconds) per stage
params.signalFrequencies = [70:10:200]; %[82    84    86    88    90    92    94    96];
...
params.sim.signalSNR   = -25;     % Signal-to-noise ratio in dB
params.duration = 90; % seconds
params.nChannels = 10;
...
[aThresholds, gThresholds] = computeBetaThresholds(params.K, params.M, params.alpha);
params.aThresholds = aThresholds;
params.gThresholds = gThresholds;

% 5. Run the sequential test (decision process)
[decisions, stageMetrics] = sequentialTest(MSCvalues, aThresholds, gThresholds, params.K);

% 6. Evaluate performance metrics (e.g., FPR and TPR)
perf = performanceMetrics(decisions, params);

% 7. Plot the results
plotResults(MSCvalues, aThresholds, gThresholds, stageMetrics, perf, params);

% 8. Print summary metrics
fprintf('Overall False Positive Rate (FPR,pct.): %.4f\n', perf.FPR);
fprintf('Overall True Positive Rate (TPR,pct.): %.4f\n', perf.TPR);
% end
