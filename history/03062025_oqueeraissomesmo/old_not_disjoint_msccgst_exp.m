...
hold on
area([M,K],[1.2*cv_max,1.2*cv_max], 'FaceColor',[0 0.4470 0.7410], 'FaceAlpha',0.3)            
...
for k=1:K
    if random_exam(k) >= aThresholds(k)
        random_exam(k+1:K) = NaN;
        last_k = k;
        break
    elseif random_exam(k) <= gThresholds(k)
        random_exam(k+1:K) = NaN;
        last_k = k;
        break
    end
...
p1 = plot(1:1:K, aThresholds, 'LineWidth',1.2, 'Color',"#77AC30"); %[0 0.4470 0.7410]);
p2 = plot(1:1:K, gThresholds,'LineWidth',1.2, 'Color',"#A2142F");%[0.8500 0.3250 0.0980]);
p3 = plot(1:1:K, random_exam,'LineWidth',1.2, 'Color',"#0072BD");
p4 = plot(last_k, random_exam(last_k),'o','MarkerSize',7, 'LineWidth',2,'Color',"#0072BD");
grid on
...
NumJ = size(MSC,2); % numero de janelas total (1 teste por janela)
for ti=1:NumJ      
    count = count+1;
    Plog = zeros(1,K);
