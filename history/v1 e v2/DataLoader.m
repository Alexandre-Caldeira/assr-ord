classdef DataLoader < handle
    ...
    methods
        function obj = DataLoader(mode, options)
            ...
            % mode: 'exp' for experimental data, 'sim' for simulated data
            % options: Name-Value pairs:
            %   'dataPath': Path to experimental .mat files
            %   'channels': Vector of channel indices to use
            %   'signalFrequencies': Vector of signal frequencies
            %   'noiseFrequencies': Vector of noise frequencies
            %   'simDuration': Duration for simulated signals (seconds)
            %   'noiseMean': Mean SNR (dB) for simulation
            %   'noiseStd': Std Dev of SNR (dB) for simulation

            arguments % Define mode and options block for Name-Value pairs
                mode {mustBeMember(mode, {'exp', 'sim'})}

                % Optional Name-Value arguments defined directly here
                options.dataPath string = 'C:\PPGEE\SBEB_CBA_24\CGST_figuras\Sinais_EEG\' % Default path
                ...
                options.noiseFrequencies (1,:) {mustBeNumeric, mustBePositive } = []
            ...
            if isempty(obj.noiseFrequencies)
                obj.generateNoiseFrequencies(); % Call helper if needed
            end
        ...
        function generateNoiseFrequencies(obj, numNoiseFreqs, range)
            % Generates random noise frequencies if not provided.
            arguments
                obj
                numNoiseFreqs = numel(obj.signalFrequencies) % Match number of signal frequencies
                range = [300, 500] % Hz range for noise frequencies
            end
            ...
            if numNoiseFreqs > availableFreqs
                warning('Requested more noise frequencies than available in range. Using replacement.');
                obj.noiseFrequencies = sort(randi(range, 1, numNoiseFreqs));
            else
                 % Ensure noise frequencies are distinct from signal frequencies
                 potentialNoise = range(1):range(2);
                 potentialNoise = setdiff(potentialNoise, obj.signalFrequencies); % Remove signal freqs
                 if numel(potentialNoise) < numNoiseFreqs
        ...
        function success = loadExperiment(obj, subjectIndex, stimulusIndex)
            % Loads data for a specific subject and stimulus.
            arguments
                obj
            ...
            try
                ...
                if isfield(data, 'x') && isfield(data, 'Fs')
                    ...
                    if max(obj.channels) > size(obj.rawSignals, 3)
                        warning('Requested channels exceed available channels in data. Using all available.');
                        obj.channels = 1:size(obj.rawSignals, 3);
                        obj.nChannels = numel(obj.channels);
                    end
                    obj.rawSignals = obj.rawSignals(:, :, obj.channels); % Keep only selected channels
                    obj.currentDuration = size(obj.rawSignals, 2); % Duration in windows/epochs
        ...
        function success = generateSimulation(obj, p)
            % Generates simulated signals for the current parameters.
            arguments
                obj
                p.duration (1,1) {mustBePositive, mustBeInteger} = obj.simDuration % Override default duration
                p.noiseMean (1,1) {mustBeNumeric} = obj.noiseMean % Override default noise mean
                p.noiseStd (1,1) {mustBeNumeric, mustBeNonnegative} = obj.noiseStd % Override default noise std
            end
            success = false;
            ...
            obj.simDuration = p.duration;
            obj.noiseMean = p.noiseMean;
            obj.noiseStd = p.noiseStd;
            ...
            obj.currentSubjectIndex = NaN; % Not applicable
            obj.currentStimulusIndex = NaN; % Not applicable
            ...
            t = (0:totalSamplesPerChannel-1)' / obj.fs; % Time vector (column)

            % Create base sinusoidal signal (sum of all signal frequencies)
            ...
            for ch = 1:obj.nChannels
                % Reshape base signal for easier epoch processing if needed,
                % but AWGN works on the whole channel signal here.
                channelSignal = reshape(baseSignal, [obj.fs, obj.simDuration]); % Samples x Epochs

                % Add noise per epoch (or per channel - simpler here)
                noisyChannelSignal = zeros(size(channelSignal));
                for epoch = 1:obj.simDuration
                    epochSignal = channelSignal(:, epoch);
                    % Apply AWGN with random SNR based on mean/std
                    currentSNR = snrFun();
        ...
        function signals = getRawSignals(obj)
            % Returns the currently loaded/generated raw signals.
            if ~obj.isDataLoaded
        ...
        function clearData(obj)
            % Clears loaded/generated signal data from memory.
            obj.rawSignals = [];
            obj.isDataLoaded = false;
        ...
        % --- Utility Methods ---
         function inspect(obj)
            % Prints current state of the DataLoader
            ...
            if strcmp(obj.mode, 'exp')
                ...
                fprintf('Subject List Size: %d\n', numel(obj.zanoteliSubjectList));
                fprintf('Stimulus List Size: %d\n', numel(obj.zanoteliStimulusNames));
                 if obj.isDataLoaded
                    fprintf('Current Subject: #%d (%s)\n', obj.currentSubjectIndex, obj.currentSubjectName);
                    fprintf('Current Stimulus: #%d (%s)\n', obj.currentStimulusIndex, obj.currentStimulusName);
                    fprintf('Loaded Duration: %d s (%d windows)\n', obj.currentDuration, size(obj.rawSignals, 2));
                 else
                     fprintf('Current Subject/Stimulus: None Loaded\n');
                 end
            else % sim mode
                fprintf('Sim. Default Duration: %d s\n', obj.simDuration);
                fprintf('Sim. Noise Mean SNR: %.1f dB\n', obj.noiseMean);
                fprintf('Sim. Noise Std Dev SNR: %.1f dB\n', obj.noiseStd);
                 if obj.isDataLoaded
                    fprintf('Current Simulation: Duration=%d s, SNR Mean=%.1f, SNR Std=%.1f\n', ...
                            obj.currentDuration, obj.noiseMean, obj.noiseStd);
                 else
                    fprintf('Current Simulation: None Generated\n');
                 end
            end
    ...
    %         % Helper to parse Name-Value pairs in constructor
    %         p = inputParser;
    %         addParameter(p, 'dataPath', obj.expDataPath, @ischar);
    %         addParameter(p, 'channels', obj.channels, @isvector);
    %         addParameter(p, 'signalFrequencies', obj.signalFrequencies, @isvector);
    %         addParameter(p, 'noiseFrequencies', obj.noiseFrequencies, @isvector);
    %         addParameter(p, 'simDuration', obj.simDuration, @(x) isscalar(x) && isnumeric(x) && x>0);
    %         addParameter(p, 'noiseMean', obj.noiseMean, @(x) isscalar(x) && isnumeric(x));
    %         addParameter(p, 'noiseStd', obj.noiseStd, @(x) isscalar(x) && isnumeric(x) && x>=0);
    %         parse(p, varargin{:});
    % 
    %         obj.expDataPath = p.Results.dataPath;
    %         obj.channels = p.Results.channels;
    %         obj.signalFrequencies = sort(p.Results.signalFrequencies);
    %         obj.noiseFrequencies = sort(p.Results.noiseFrequencies);
    %         obj.simDuration = round(p.Results.simDuration);
    %         obj.noiseMean = p.Results.noiseMean;
    %         obj.noiseStd = p.Results.noiseStd;
    %     end
    % end
end